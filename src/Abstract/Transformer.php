...
abstract class Transformer
{
    ...
    protected function transformValue($value, array $config): mixed
    {
        if (isset($config['group_value'])) {
            ...
            if ($config['group_value'] == 'split') {
                $arrGroupVal = explode($this->groupValueSeparator, $value);
                $value = $arrGroupVal[$position] ?? null;
                ...
                if (!is_array($temp)) {
                    ...
                    if (((int) $config['group_value_position'] - count($arr)) === 0) $arr[] = $value;
    ...
    protected function transformDate(mixed $value, array $config): string
    {
        ...
        foreach ($formats as $format) {
            ...
            if ($dateTime) return $dateTime->format($config['format'] ?? 'd/m/Y');
        }
